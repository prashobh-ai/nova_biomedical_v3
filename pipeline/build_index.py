...
def build(source_dir: Path, fda_dir: Path, out_path: Path, semantic_path: Path,
          youtube_dir: Path | None = None) -> dict:
    print("=" * 66)
    ...
    connectors = registry_from({"documents": source_dir, "fda_structured": fda_dir,
                                "youtube": youtube_dir})
    if not connectors:
        print(f"[!] No sources found ({source_dir}, {fda_dir}, {youtube_dir})", file=sys.stderr)
        sys.exit(1)
    ...
    # One representative record per document, and the chunk tally, in a single
    # pass. Date resolution then runs exactly once per document below.
    #
    # This used to be a dict.setdefault() whose default argument called
    # resolve_date(). setdefault evaluates that argument EAGERLY, on every
    # iteration, so the date was resolved once per *chunk* rather than once per
    # *document* - re-opening and re-parsing the same PDF thousands of times.
    # With a few dozen small PDFs that was merely wasteful; with 82 documents,
    # one of them 32 MB, it exhausts memory and the build is OOM-killed.
    first_rec: dict = {}
    chunk_tally: dict = defaultdict(int)
    for c, rec in zip(chunks, aligned):
        if c.document_name not in first_rec:
            first_rec[c.document_name] = (c, rec)
        chunk_tally[c.document_name] += 1

    for name, (c, rec) in first_rec.items():
        dates = resolve_date(
            source_path=_Path(rec.metadata["source_path"])
            if rec.metadata.get("source_path") else None,
            text=doc_text_head.get(name, ""),
            ...
            published_date=rec.metadata.get("published_at") or None,
            date_method=rec.metadata.get("date_method"),
        )
        documents_meta[name] = {
            "id": c.document_id, "name": name,
            ...
            "chunk_count": chunk_tally[name],
            **{f"date_{k}": v for k, v in dates.items()},
        }
...
def main():
    ...
    ap.add_argument("--source", type=Path, default=Path("knowledge_fabric"))
    ap.add_argument("--fda", type=Path,
                    default=Path("knowledge_fabric/fda_files/structured"))
    ap.add_argument("--youtube", type=Path,
                    default=Path("knowledge_fabric/youtube"))
    ...
    args = ap.parse_args()
    build(args.source, args.fda, args.out, args.semantic_out, args.youtube)
