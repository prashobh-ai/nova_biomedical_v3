...
PUBLISHED_DATE = "channel_publication_date"
OBSERVED_DATE = "first_observed_by_scheduler"
UNKNOWN = "unknown"
...
def resolve(*, source_path: Path | None = None, text: str = "",
            ...
            date_method: str | None = None) -> dict:
    """Resolve a document date with its provenance."""
    # Published media. A channel publication date is asserted by the publisher
    # and cannot be silently changed by re-saving a file, so it outranks PDF
    # metadata. A date the scheduler merely observed is recorded as a separate,
    # lower-confidence method - never laundered into a publication date.
    if published_date and ISO_RE.fullmatch(published_date):
        method = OBSERVED_DATE if date_method == OBSERVED_DATE else PUBLISHED_DATE
        return {"date": published_date, "method": method,
                "confidence": CONFIDENCE[method],
                "explanation": ("Date the scheduler first observed this video on the "
                                "channel; an upper bound, not a publisher assertion."
                                if method == OBSERVED_DATE else
                                "Publication date on the company's own channel. "
                                "Publisher-asserted, not a file timestamp.")}
