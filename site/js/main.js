...
// The YouTube glyph, inline so a citation can carry it without a network fetch
// and without a sprite sheet. Kept small - it marks a source, it is not branding.
...
// Watch the source. Every video cited in the current answer gets a player,
// seeked to the second its sentence was spoken. Verification for a video is
...
function renderVideoSources(citations) {
  ...
  if (!byVideo.size) { host.hidden = true; host.innerHTML = ''; return; }

  const cards = [...byVideo.values()].map(v => {
    const stamp = clockLabel(v.start);
    const watch = `https://www.youtube.com/watch?v=${v.id}${v.start ? '&t=' + v.start + 's' : ''}`;
    const embed = `https://www.youtube-nocookie.com/embed/${v.id}?start=${v.start}&rel=0&modestbranding=1`;
    return `
      <figure class="video-source">
        <div class="video-source-frame">
          <iframe src="${embed}" title="${escapeHtml(v.title)}" loading="lazy"
        ...
        <figcaption class="video-source-meta">
          ...
          <div class="video-source-sub">
            ...
            ${v.transcribed
              ? `<span class="video-source-stamp">plays from ${escapeHtml(stamp)}</span>
                 <span class="lineage-tier is-transcribed">auto-transcribed</span>`
              : `<span class="lineage-tier is-authored">publisher-written</span>`}
          </div>
          <a class="video-source-link" href="${watch}" target="_blank" rel="noopener noreferrer">
            Open on YouTube${v.transcribed ? ' at ' + escapeHtml(stamp) : ''} &rarr;
          </a>
        </figcaption>
      </figure>`;
  }).join('');
  ...
  host.innerHTML = `
    <div class="video-sources">
      <div class="video-sources-head">
        ${YT_MARK}
        <span>Watch the source</span>
        <span class="video-sources-count">${byVideo.size} video${byVideo.size > 1 ? 's' : ''} cited</span>
      </div>
